...
CREATE RULE-TABLE RULE-TABLE-LENGTH ALLOT
...
: VERSION ( -- )
  ." celt version 1.4 copyright 2025 Midnight Salmon" ;
...
: ALIVE? ( index -- f )
  GRID + C@ ALIVE C@ 0= IF DROP 0 ELSE
  0 SWAP ALIVE C@ 0 DO DUP ALIVE I + 1+ C@ = ROT OR SWAP LOOP DROP
  THEN IF 1 ELSE 0 THEN ;

: MOORE ( index -- neighbourhood )
  DUP 1 LEFT 1 UP ALIVE? >R
  DUP 1 UP ALIVE? >R
  DUP 1 UP 1 RIGHT ALIVE? >R
  DUP 1 LEFT ALIVE? >R
  DUP 1 RIGHT ALIVE? >R
  DUP 1 LEFT 1 DOWN ALIVE? >R
  DUP 1 DOWN ALIVE? >R
  1 DOWN 1 RIGHT ALIVE? >R
  R> R> R> R> R> R> R> R> + + + + + + + ;
